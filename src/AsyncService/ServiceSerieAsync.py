...
class ServiceSerieAsync(ServiceTMDB):
    ...
    async def _fetch(self, p_id, session):
        url = self.base_url + "tv/" + str(p_id) + "?api_key=" + self.api_key + "&language=" + self.language + "&append_to_response=credits"
        self.p_bar.update()
        async with session.get(url) as reponse:
            if reponse.status == 200:
                json = await reponse.json()
                return Serie(json).save(self.orm_session)
    ...
    @property
    def max(self):
        response = requests.get(self.base_url + "tv/latest?api_key=" + self.api_key + "&language=" + self.language + "&append_to_response=credits")
        if response.status_code == 200:
            video = Serie(response.json())
            return video.id_video

    @property
    def min(self):
        return (self.orm_session.query(Serie).order_by(text("id_video desc, id_video")).first().id_video + 1)if self.orm_session.query(Serie).count() > 0 else 0
